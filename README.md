...
- **Chat UI** - веб-интерфейс для чата с автозапуском бэкендов
...
├── venv-vllm/          # Python окружение (создаётся при установке)
├── llama.cpp/          # llama.cpp (клонируется отдельно)
├── models/             # Скачанные модели
│   ├── hf/             # HuggingFace модели (для vLLM)
│   └── gguf/           # GGUF модели (для llama.cpp)
├── chat/               # Веб-интерфейс для чата
│   ├── README.md       # Документация Chat UI
│   ├── app.py          # Flask сервер
│   └── ...             # Остальные модули
...
Мы используем `uv` для быстрого управления зависимостями, но можно использовать и стандартный `pip`.
...
Веб-интерфейс для удобного чата с моделями. Подробная документация в [chat/README.md](chat/README.md).
...
- **Автозапуск бэкендов** - выберите модель и бэкенд запустится автоматически
- **Стриминг ответов** - текст появляется в реальном времени
- **Отображение reasoning** - показывает процесс мышления модели
- **История чатов** - диалоги сохраняются в сайдбаре
...
```
┌─────────────────────────────────────────────────────────────────┐
│  [+] New Chat          LLM Chat       Backend: [vLLM ▼] [●]    │
├──────────────┬──────────────────────────────────────────────────┤
│  Chat 1      │  User: Hello!                                   │
│  Chat 2  ←   │                                                  │
│              │  ▼ Thinking...                                   │
│              │    Let me respond to the greeting...             │
│              │  ─────────────────────────────────────────────── │
│              │  Assistant: Hi! How can I help you today?        │
├──────────────┼──────────────────────────────────────────────────│
│              │  Model: [Qwen3-8B-AWQ ▼]    [⚙ Settings]         │
│              │  [Type a message...                       ] [➤]  │
└──────────────┴──────────────────────────────────────────────────┘
```
...
- [x] ~~WebUI для чата с моделями~~ (Chat UI добавлен)
